...
export default async function ServicePageRoute({ params }: Props) {
  const service = getServicePage((await params).service);
  if (!service) notFound();
  const serviceIndex = servicePages.findIndex(({ slug }) => slug === service.slug);
  const otherService = servicePages[(serviceIndex + 1) % servicePages.length];
  const caseStudy = "caseStudy" in service ? service.caseStudy : null;
  const advisoryExample = "advisoryExample" in service ? service.advisoryExample : null;
  ...
  return <>
    ...
    <main id="main-content">
      ...
      {advisoryExample && <section className="section section-white advisory-example" aria-labelledby="advisory-example-heading"><div className="shell advisory-example-grid"><figure><Image src={advisoryExample.image.src} alt={advisoryExample.image.alt} width={advisoryExample.image.width} height={advisoryExample.image.height} sizes="(max-width: 760px) 100vw, 42vw" /><figcaption>Inspection photograph — advice provided; replacement not undertaken</figcaption></figure><div><p className="section-label">Inspection finding • Advice only</p><h2 id="advisory-example-heading">{advisoryExample.title}</h2><p>{advisoryExample.summary}</p><div className="guidance-note" role="note" aria-label="Outcome of leadwork advice"><strong>Not repaired by Vallano</strong><p>{advisoryExample.advice}</p></div></div></div></section>}

      {caseStudy ? <section className="section work-example case-file service-case-file" aria-labelledby="service-case-study-heading"><div className="shell"><p className="section-label">{"label" in caseStudy ? caseStudy.label : "Verified completed work"}</p><h2 id="service-case-study-heading">{caseStudy.title}</h2><p className="case-file-summary">{caseStudy.summary}</p><div className="work-example-grid"><article><span>01</span><h3>Problem reported</h3><p>{caseStudy.problem}</p></article><article><span>02</span><h3>What the inspection found</h3><p>{caseStudy.inspection}</p></article><article><span>03</span><h3>Completed work</h3><p>{caseStudy.outcome}</p></article></div><div className="case-file-details"><article><p className="case-file-kicker">How the work was completed</p><h3>{caseStudy.repairTitle}</h3><p>{caseStudy.repair}</p><p>{caseStudy.detail}</p></article><div className="case-file-specification" role="group" aria-labelledby="service-materials-heading"><p className="case-file-kicker">Access and materials</p><h3 id="service-materials-heading">Project specification</h3><ul>{caseStudy.materials.map((material) => <li key={material}>{material}</li>)}</ul></div></div><div className={`case-file-gallery service-case-gallery service-case-gallery-${caseStudy.images.length} service-case-gallery-${caseStudy.images[0].height > caseStudy.images[0].width ? "portrait" : "landscape"}`} aria-label={`${caseStudy.title} photographs`}>{caseStudy.images.map((image, index) => <figure key={image.src}><Image src={image.src} alt={image.alt} width={image.width} height={image.height} sizes="(max-width: 760px) 100vw, 50vw" /><figcaption><b>{String(index + 1).padStart(2, "0")}</b><span>{image.caption}</span></figcaption></figure>)}</div><div className="guidance-note service-case-note" role="note" aria-label="Important repair note"><strong>Important</strong><p>{service.note}</p></div></div></section> : "evidence" in service && <section className="section section-white service-evidence" aria-labelledby="service-evidence-heading"><div className="shell"><p className="section-label">Real work. Clear context.</p><h2 id="service-evidence-heading">{service.evidenceTitle}</h2><p className="section-intro">{service.evidenceIntroduction}</p><div className={`service-evidence-grid service-evidence-grid-${service.evidence.length}`}>{service.evidence.map((item) => <article key={item.title}><Image src={item.image.src} alt={item.image.alt} width={item.image.width} height={item.image.height} sizes="(max-width: 760px) 100vw, 33vw" /><div><span>{item.area}</span><h3>{item.title}</h3><p>{item.text}</p><Link href={item.href}>{item.linkLabel} <b aria-hidden="true">→</b></Link></div></article>)}</div><div className="guidance-note" role="note" aria-label="Important repair note"><strong>Important</strong><p>{service.note}</p></div></div></section>}

      <section className="section section-light" aria-labelledby="service-areas-heading"><div className="shell"><p className="section-label">Priority local service areas</p><h2 id="service-areas-heading">{service.name} in Christleton, Rowton and Waverton</h2><p className="section-intro">Vallano Roofing focuses this service across Christleton, Rowton and Waverton. Choose your village for local roof-repair information, homeowner guidance and relevant completed-work evidence.</p><div className="area-grid service-area-grid">{locations.map((location) => <article key={location.slug}><span>{location.postcode}</span><h3>{service.name} in {location.name}</h3><p>Read local roofing guidance and see how Vallano approaches roof repair enquiries from homeowners in {location.name}.</p><Link href={locationPath(location)}>Roof repairs in {location.name} <b aria-hidden="true">→</b></Link></article>)}</div></div></section>
