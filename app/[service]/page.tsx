...
export default async function ServicePageRoute({ params }: Props) {
  ...
  return <>
    ...
    <main id="main-content">
      ...
      <section className="section section-light" aria-labelledby="service-areas-heading"><div className="shell"><p className="section-label">Local service areas</p><h2 id="service-areas-heading">{service.name} across the priority service area</h2><p className="section-intro">Vallano Roofing is Chester-based and welcomes suitable enquiries from Christleton, Rowton and Waverton.</p><div className="area-grid service-area-grid">{locations.map((location) => <article key={location.slug}><span>{location.postcode}</span><h3>{service.name} in {location.name}</h3><p>View local roof-repair information, homeowner guidance and documented Vallano work relevant to {location.name}.</p><Link href={locationPath(location)}>View {location.name} roof repairs <b aria-hidden="true">→</b></Link></article>)}</div></div></section>
