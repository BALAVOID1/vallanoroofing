...
export default async function LocationPage({ params }: Props) {
  ...
  return <>
    ...
    <main id="main-content">
      ...
      <section className="section section-white location-service-links" aria-labelledby="specialist-services-heading"><div className="shell"><p className="section-label">Detailed service information</p><h2 id="specialist-services-heading">Roof repair services in {location.name}</h2><p className="section-intro">Read how Vallano assesses each roof detail, confirms a suitable scope and prioritises a targeted repair where the condition allows.</p><div className="service-link-grid">{servicePages.map((service) => <Link className="button" key={service.slug} href={servicePath(service)}>{service.name} in {location.name} <span aria-hidden="true">→</span></Link>)}</div></div></section>
