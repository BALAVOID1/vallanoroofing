...
export default async function LocationPage({ params }: Props) {
  ...
  return <>
    ...
    <main id="main-content">
      ...
      <section className="section section-white location-service-links" aria-labelledby="specialist-services-heading"><div className="shell location-intro"><div><p className="section-label">Detailed service information</p><h2 id="specialist-services-heading">Slate and chimney flashing repairs in {location.name}</h2></div><div><p>Read how Vallano approaches damaged slate coverings and confirmed defects at chimney flashing details in {location.name}. A leak near a chimney is not automatically caused by its flashing, so each service page explains how the likely cause informs the repair scope.</p><div className="button-row"><Link className="button button-brand" href="/slate-roof-repairs">Slate roof repairs in {location.name}</Link><Link className="button service-link-dark" href="/chimney-flashing-repairs">Chimney flashing repairs in {location.name}</Link></div></div></div></section>
